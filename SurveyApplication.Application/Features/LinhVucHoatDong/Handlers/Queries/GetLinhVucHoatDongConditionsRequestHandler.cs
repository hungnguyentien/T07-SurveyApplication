...
namespace SurveyApplication.Application.Features.LinhVucHoatDong.Handlers.Queries
{
    public class GetLinhVucHoatDongConditionsRequestHandler : BaseMasterFeatures, IRequestHandler<GetLinhVucHoatDongConditionsRequest, BaseQuerieResponse<LinhVucHoatDongDto>>
    {
        ...
        {
            var LinhVucHoatDongs = await _surveyRepo.LinhVucHoatDong.GetByConditionsQueriesResponse(request.PageIndex, request.PageSize, x => (string.IsNullOrEmpty(request.Keyword) || !string.IsNullOrEmpty(x.MaLinhVuc) && x.MaLinhVuc.Contains(request.Keyword) || !string.IsNullOrEmpty(x.TenLinhVuc) && x.TenLinhVuc.Contains(request.Keyword)) && x.Deleted == false, "");
