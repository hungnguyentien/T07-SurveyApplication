...
{
    public class GetCauHoiConditionsRequestHandler : BaseMasterFeatures, IRequestHandler<GetCauHoiConditionsRequest, BaseQuerieResponse<CauHoiDto>>
    {
        private readonly IMapper _mapper;
        public GetCauHoiConditionsRequestHandler(ISurveyRepositoryWrapper surveyRepository, IMapper mapper): base(surveyRepository)
        {
            _mapper = mapper;
        }
        ...
        {
            var lstCauHoi = await _surveyRepo.CauHoi.GetByConditionsQueriesResponse(request.PageIndex, request.PageSize, x => (string.IsNullOrEmpty(request.Keyword) || x.TieuDe.Contains(request.Keyword)) && x.ActiveFlag == (int)EnumCommon.ActiveFlag.Active && !x.Deleted, request.OrderBy ?? "");
