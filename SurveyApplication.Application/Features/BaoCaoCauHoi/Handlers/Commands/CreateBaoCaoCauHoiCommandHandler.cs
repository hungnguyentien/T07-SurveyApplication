...
public class CreateBaoCaoCauHoiCommandHandler : BaseMasterFeatures,
    IRequestHandler<CreateBaoCaoCauHoiCommand, BaseCommandResponse>
{
    ...
    public async Task<BaseCommandResponse> Handle(CreateBaoCaoCauHoiCommand request,
        CancellationToken cancellationToken)
    {
        var idGuiEmail =
            JsonConvert.DeserializeObject<EmailThongTinChungDto>(
                StringUltils.DecryptWithKey(request.IdGuiEmail, EmailSettings.SecretKey))?.IdGuiEmail ?? 0;
