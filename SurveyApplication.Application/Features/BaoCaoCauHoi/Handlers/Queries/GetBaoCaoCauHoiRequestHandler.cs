...
namespace SurveyApplication.Application.Features.BaoCaoCauHoi.Handlers.Queries
{
    public class GetBaoCaoCauHoiRequestHandler : BaseMasterFeatures, IRequestHandler<GetBaoCaoCauHoiRequest, ThongKeBaoCaoDto>
    {
        ...
        public async Task<ThongKeBaoCaoDto> Handle(GetBaoCaoCauHoiRequest request, CancellationToken cancellationToken)
        {
            ...
            var groupedResults = queryBaoCao.GroupBy(g => new { g.IdCauHoi, g.CauHoi, g.LoaiCauHoi }).OrderBy(o => o.Key.IdCauHoi);
            ...
            foreach (var item in groupedDataList.Where(item => item.CauHoiTraLoi != null))
            {
                ...
                else //TODO Câu trả lời dạng chọn nhiều check khác (vì câu trả lời theo câu hỏi phụ có thể giống nhau)
                {
                    foreach (var itemCauHoiTraLoi in item.CauHoiTraLoi)
                    {
                        ...
                        var lstCauTraLoiNew = (from itemCauTraLoi in lstCauTraLoi?.Where(x => !string.IsNullOrEmpty(x)) ?? new List<string>() select $"{itemCauTraLoi} ({itemCauHoiTraLoi.CauHoiPhu})").ToList();
            ...
            var lstDoiTuongThamGiaKs = await _surveyRepo.LoaiHinhDonVi.GetAllQueryable().Where(x => !x.Deleted && query.Select(q => q.IdLoaiHinhDonVi).Contains(x.Id)).Select(x => new DoiTuongThamGiaKs
            {
                Ten = x.TenLoaiHinh,
                SoLuong = query.Select(cauHoiTraLoi => new { cauHoiTraLoi.IdLoaiHinhDonVi, cauHoiTraLoi.IdBangKhaoSat }).GroupBy(gr => new { gr.IdLoaiHinhDonVi, gr.IdBangKhaoSat }).Count(q => q.Key.IdLoaiHinhDonVi == x.Id)
            }).Take(5).OrderByDescending(x => x.SoLuong).ToListAsync(cancellationToken: cancellationToken);
