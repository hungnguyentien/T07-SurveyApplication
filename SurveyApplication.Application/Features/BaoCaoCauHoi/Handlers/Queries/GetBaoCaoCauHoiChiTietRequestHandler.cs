...
public class GetBaoCaoCauHoiChiTietRequestHandler : BaseMasterFeatures,
    IRequestHandler<GetBaoCaoCauHoiChiTietRequest, BaseQuerieResponse<BaoCaoCauHoiChiTietDto>>
{
    public GetBaoCaoCauHoiChiTietRequestHandler(ISurveyRepositoryWrapper surveyRepository) : base(surveyRepository)
    {
    }

    public async Task<BaseQuerieResponse<BaoCaoCauHoiChiTietDto>> Handle(GetBaoCaoCauHoiChiTietRequest request,
        CancellationToken cancellationToken)
    {
        var query = from a in _surveyRepo.BaoCaoCauHoi.GetAllQueryable()
            ...
            where a.IdDotKhaoSat == request.IdDotKhaoSat &&
                  ...
                  (request.NgayBatDau == null ||
                   b.NgayBatDau.Date >= request.NgayBatDau.GetValueOrDefault(DateTime.MinValue).Date) &&
                  (request.NgayKetThuc == null ||
                   b.NgayKetThuc.Date <= request.NgayKetThuc.GetValueOrDefault(DateTime.MaxValue).Date) &&
        ...
        var data = from a in query
            group new { a } by new { a.IdBangKhaoSat, a.IdDotKhaoSat, a.IdDonVi, a.TenDaiDienCq, a.IdGuiEmail }
            into grBc
            select new BaoCaoCauHoiChiTietDto
            {
                ...
                DauThoiGian =
                    query.First(x =>
                        x.IdDonVi == grBc.Key.IdDonVi && x.IdBangKhaoSat == grBc.Key.IdBangKhaoSat &&
                        x.IdDotKhaoSat == grBc.Key.IdDotKhaoSat).DauThoiGian ?? DateTime.MinValue,
                LstCauHoiCauTraLoi = query
                    .Where(x => x.IdDonVi == grBc.Key.IdDonVi && x.IdBangKhaoSat == grBc.Key.IdBangKhaoSat &&
                                x.IdDotKhaoSat == grBc.Key.IdDotKhaoSat).GroupBy(x => new { x.LoaiCauHoi, x.CauHoi })
                    .Select(x =>
                        new CauHoiCauTraLoiChiTietDto
                        {
                            ...
                            CauTraLoi = x
                                .Where(ctl =>
                                    !string.IsNullOrEmpty(ctl.CauTraLoi) && ctl.LoaiCauHoi == x.Key.LoaiCauHoi &&
                                    ctl.CauHoi == x.Key.CauHoi).GroupBy(ctl => new { ctl.CauTraLoi })
                                .Select(ctl => ctl.Key.CauTraLoi).ToList()
                        }).ToList()
            };

        var totalCount = (await data.ToListAsync(cancellationToken)).LongCount();
