...
namespace SurveyApplication.Application.Features.Role.Handlers.Queries
{
    public class GetRoleDetailRequestHandler : BaseMasterFeatures, IRequestHandler<GetRoleDetailRequest, UpdateRoleDto>
    {
        ...
        public GetRoleDetailRequestHandler(ISurveyRepositoryWrapper surveyRepository, RoleManager<Domain.Role> roleManager) : base(surveyRepository)
        {
        ...
        {
            var role = await _surveyRepo.Role.FirstOrDefaultAsync(x => !x.Deleted && x.Id == request.Id) ?? new Domain.Role();
