        ...
        CancellationToken cancellationToken)
    {
        var NguoiDaiDiens = await _surveyRepo.NguoiDaiDien.GetByConditionsQueriesResponse(request.PageIndex,
            ...
            x => (string.IsNullOrEmpty(request.Keyword) ||
                  (!string.IsNullOrEmpty(x.MaNguoiDaiDien) && x.MaNguoiDaiDien.Contains(request.Keyword)) ||
                  (!string.IsNullOrEmpty(x.HoTen) && x.HoTen.Contains(request.Keyword))) && x.Deleted == false, "");
