...
public class GetGuiEmailBksConditionsRequestHandler : BaseMasterFeatures,
    IRequestHandler<GetGuiEmailBksConditionsRequest, BaseQuerieResponse<GuiEmailBksDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        var query = from a in _surveyRepo.BangKhaoSat.GetAllQueryable().AsNoTracking()
            ...
            where (!a.Deleted && !b.Deleted
                              && string.IsNullOrEmpty(request.Keyword)) || a.TenBangKhaoSat.Contains(request.Keyword) ||
                  a.MaBangKhaoSat.Contains(request.Keyword)
            select new
            {
                a.MaBangKhaoSat,
                a.TenBangKhaoSat,
        ...
        var data = from a in query
            group new { a } by new { a.IdBangKhaoSat, a.NgayBatDau, a.NgayKetThuc, a.MaBangKhaoSat, a.TenBangKhaoSat }
            into gbks
            select new GuiEmailBksDto
            {
                ...
                CountSendThanhCong = query.Count(x =>
                    x.IdBangKhaoSat == gbks.Key.IdBangKhaoSat && x.TrangThai == (int)EnumGuiEmail.TrangThai.ThanhCong),
                CountSendLoi = query.Count(x =>
                    x.IdBangKhaoSat == gbks.Key.IdBangKhaoSat && x.TrangThai == (int)EnumGuiEmail.TrangThai.GuiLoi),
                CountSendThuHoi = query.Count(x =>
                    x.IdBangKhaoSat == gbks.Key.IdBangKhaoSat && x.TrangThai == (int)EnumGuiEmail.TrangThai.ThuHoi),
