...
public class GuiLaiGuiEmailCommandHandler : BaseMasterFeatures,
    IRequestHandler<GuiLaiGuiEmailCommand, BaseCommandResponse>
{
    private readonly IEmailSender _emailSender;
    private readonly IMapper _mapper;

    public GuiLaiGuiEmailCommandHandler(ISurveyRepositoryWrapper surveyRepository, IMapper mapper,
    ...
    {
        var lstGuiEmail =
            (await _surveyRepo.GuiEmail.GetAllListAsync(x =>
                !x.Deleted && request.LstIdGuiMail.Contains(x.Id) && x.TrangThai == (int)EnumGuiEmail.TrangThai.GuiLoi))
            .ToList();

        var lstGuiEmailThuHoiOld =
            (await _surveyRepo.GuiEmail.GetAllListAsync(x =>
                !x.Deleted && request.LstIdGuiMail.Contains(x.Id) && x.TrangThai == (int)EnumGuiEmail.TrangThai.ThuHoi))
            .ToList();
        ...
        if (lstGuiEmailThuHoiOld.Any())
        {
            ...
            var lstDonVi =
                (await _surveyRepo.DonVi.GetAllListAsync(x =>
                    request.GuiEmailDto.LstIdDonVi.Contains(x.Id) && !x.Deleted)).ToList();
            if (!lstDonVi.Any())
        ...
        {
            lstGuiEmailThuHoiOld.ForEach(guiEmail => { guiEmail.Deleted = true; });
