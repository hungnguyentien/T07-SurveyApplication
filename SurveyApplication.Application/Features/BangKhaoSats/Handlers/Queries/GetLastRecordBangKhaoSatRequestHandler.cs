...
namespace SurveyApplication.Application.Features.BangKhaoSats.Handlers.Queries
{
    public class GetLastRecordBangKhaoSatRequestHandler : BaseMasterFeatures, IRequestHandler<GetLastRecordBangKhaoSatRequest, string>
    {
        ...
        public GetLastRecordBangKhaoSatRequestHandler(ISurveyRepositoryWrapper surveyRepository, IMapper mapper, ILoggerManager logger) : base(
            surveyRepository)
        {
            _mapper = mapper;
            _logger = logger;
        ...
        {
            var lastEntity = await _surveyRepo.BangKhaoSat.GetAllQueryable().OrderByDescending(e => e.Id).FirstOrDefaultAsync(cancellationToken: cancellationToken);
            try
            {
                if (lastEntity != null)
                {
