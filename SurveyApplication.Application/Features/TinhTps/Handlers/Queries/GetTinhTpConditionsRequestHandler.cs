...
namespace SurveyApplication.Application.Features.TinhTps.Handlers.Queries
{
    ...
    public class GetTinhTpConditionsRequestHandler : BaseMasterFeatures, IRequestHandler<GetTinhTpConditionsRequest, BaseQuerieResponse<TinhTpDto>>
    {
        ...
        {
            var TinhTps = await _surveyRepo.TinhTp.GetByConditionsQueriesResponse(request.PageIndex, request.PageSize, x => (string.IsNullOrEmpty(request.Keyword) || !string.IsNullOrEmpty(x.Code) && x.Code.Contains(request.Keyword) || !string.IsNullOrEmpty(x.Name) && x.Name.Contains(request.Keyword)) && x.Deleted == false, "");
