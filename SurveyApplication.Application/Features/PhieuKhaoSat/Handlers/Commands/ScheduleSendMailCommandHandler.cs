...
public class ScheduleSendMailCommandHandler : BaseMasterFeatures,
    IRequestHandler<ScheduleSendMailCommand, BaseCommandResponse>
{
    ...
    {
        var lstGuiEmail = await _surveyRepo.GuiEmail.GetAllQueryable().AsNoTracking()
            .Where(x => !x.Deleted && (x.TrangThai == (int)EnumGuiEmail.TrangThai.DangGui ||
                                       x.TrangThai == (int)EnumGuiEmail.TrangThai.GuiLoi)).Take(10)
        ...
        var lstDonVi = await _surveyRepo.DonVi.GetAllQueryable().AsNoTracking()
            .Where(x => !x.Deleted && lstGuiEmail.Select(gm => gm.IdDonVi).Contains(x.Id))
            .Select(x => new { x.Id, x.MaDonVi }).ToListAsync(cancellationToken);
        ...
        foreach (var guiEmail in lstGuiEmail)
            if (string.IsNullOrEmpty(guiEmail.NoiDung))
            {
                ...
                var bodyHtml = externalHtmlContent.Replace("{{LINK_SEND_MAIL}}",
                    $"{EmailSettings.DomainKhaoSat}/khao-sat?maDoanhNghiep={mdv}");
    ...
    private async Task RunTasks(IEnumerable<Domain.GuiEmail> lstGuiEmail, IList<byte[]?> lstAttachment,
        IList<string> lstAttachmentName)
    {
        var tasks = lstGuiEmail.Select(guiEmail => Task.Run(() => DoWork(guiEmail, lstAttachment, lstAttachmentName)))
            .ToList();
    ...
    {
        var resultSend = await _emailSender.SendEmail(guiEmail.NoiDung, guiEmail.TieuDe, guiEmail.DiaChiNhan,
            lstAttachment, lstAttachmentName);
