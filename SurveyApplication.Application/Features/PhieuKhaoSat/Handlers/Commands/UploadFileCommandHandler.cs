...
public class UploadFileCommandHandler : BaseMasterFeatures, IRequestHandler<UploadFileCommand, List<StgFileDto>>
{
    ...
    public UploadFileCommandHandler(ISurveyRepositoryWrapper surveyRepository, IMapper mapper,
        IOptions<StorageConfigs> storageConfigs) : base(surveyRepository)
    {
        _mapper = mapper;
    ...
    public async Task<List<StgFileDto>> Handle(UploadFileCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var file in uploadFileDto.Files)
        {
            ...
            var yearMonthDayPath = Path.Combine(StorageConfigs.BaseStorage, DateTime.Now.Year.ToString(),
