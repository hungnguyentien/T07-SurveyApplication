...
public class ScheduleUpdateStatusCommandHandler : BaseMasterFeatures,
    IRequestHandler<ScheduleUpdateStatusCommand, BaseCommandResponse>
{
    public ScheduleUpdateStatusCommandHandler(ISurveyRepositoryWrapper surveyRepository) : base(surveyRepository)
    {
    }

    public async Task<BaseCommandResponse> Handle(ScheduleUpdateStatusCommand request,
        CancellationToken cancellationToken)
    {
        var hoanThanhDks = await _surveyRepo.DotKhaoSat.GetAllListAsync(x =>
            !x.Deleted && x.NgayKetThuc.Date.AddDays(1) <= DateTime.Now.Date &&
            x.TrangThai != (int)EnumDotKhaoSat.TrangThai.HoanThanh);
        if (hoanThanhDks.Any())
        {
            //TODO check đợt khảo sát có bảng khảo sát mới update
            var hasBks = await _surveyRepo.BangKhaoSat.GetAllQueryable()
                .Where(x => !x.Deleted && hoanThanhDks.Select(dks => dks.Id).Contains(x.IdDotKhaoSat))
                .Select(x => x.IdDotKhaoSat)
        ...
        var hoanThanhBks = await _surveyRepo.BangKhaoSat.GetAllListAsync(x =>
            !x.Deleted && x.NgayKetThuc.Date.AddDays(1) <= DateTime.Now.Date &&
            x.TrangThai != (int)EnumBangKhaoSat.TrangThai.HoanThanh);
        if (hoanThanhBks.Any())
        {
