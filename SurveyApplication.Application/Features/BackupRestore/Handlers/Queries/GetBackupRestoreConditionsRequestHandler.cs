...
{
    public class GetBackupRestoreConditionsRequestHandler : BaseMasterFeatures, IRequestHandler<GetBackupRestoreConditionsRequest, BaseQuerieResponse<BackupRestoreDto>>
    {
        #region Properties
        ...
        public GetBackupRestoreConditionsRequestHandler(ISurveyRepositoryWrapper surveyRepository, ILoggerManager logger, IOptions<BackupRestoreConfiguration> backuOptions) : base(
            surveyRepository)
        {
            _logger = logger;
        ...
        public Task<BaseQuerieResponse<BackupRestoreDto>> Handle(GetBackupRestoreConditionsRequest request, CancellationToken cancellationToken)
        {
            ...
            var result = data.OrderByDescending(x => x.CreateDateTime).Skip((request.PageIndex - 1) * request.PageSize).Take(request.PageSize).ToList();
