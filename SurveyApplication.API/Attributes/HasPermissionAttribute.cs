...
public class HasPermissionAttribute : AuthorizeAttribute, IAsyncAuthorizationFilter
{
    ...
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        try
        {
            ...
            var rolesInToken = decodedToken.Claims.Where(x => x.Type == ClaimsIdentity.DefaultRoleClaimType)
                .Select(x => x.Value);
            ...
            var isAccess = decodedToken.Claims.Any(x =>
                _codeModules.Select(m => m.ToString()).Contains(x.Type) && _permissions.All(p =>
                    JsonExtensions.DeserializeFromJson<List<int>>(x.Value).Contains(p)));
            if (!isAccess)
                context.Result = new ForbidResult(); // Forbidden
        }
    ...
    private string? GetToken(AuthorizationFilterContext context)
    {
        ...
        {
            false when auth != null && auth.ToLower().Contains("bearer") => Regex.Replace(auth,
                "bearer |Bearer |bearer_|Bearer_",
                ""),
            false => auth,
            _ => token
        };
