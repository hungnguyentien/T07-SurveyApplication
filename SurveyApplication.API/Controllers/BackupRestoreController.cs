...
public class BackupRestoreController : ControllerBase
{
    ...
    public async Task<ActionResult<BaseQuerieResponse<BackupRestoreDto>>> GetByConditionBangKhaoSat(
        [FromQuery] Paging paging)
    {
        var lstBackupRestore = await _mediator.Send(new GetBackupRestoreConditionsRequest
    ...
    {
        var result = EnumUltils.GetDescription<EnumBackupStore.ScheduleDayofweek>()
            .Select(x => new { text = x.Value, value = ((int)x.Key).ToString() }).ToList();
        return Ok(result);
    }
    ...
    {
        var result = EnumUltils.GetDescription<EnumBackupStore.ScheduleHour>()
            .Select(x => new { text = x.Value, value = ((int)x.Key).ToString() }).ToList();
        return Ok(result);
    }
    ...
    {
        var result = EnumUltils.GetDescription<EnumBackupStore.ScheduleMinute>()
            .Select(x => new { text = x.Value, value = ((int)x.Key).ToString() }).ToList();
        return Ok(result);
    }
}