...
public class BangKhaoSatController : ControllerBase
{
    ...
    [HttpGet("GetAll")]
    [HasPermission(new[] { (int)EnumModule.Code.QlKs, (int)EnumModule.Code.TkKs },
        new[] { (int)EnumPermission.Type.Read })]
    ...
    public async Task<ActionResult<BaseQuerieResponse<BangKhaoSatDto>>> GetByConditionBangKhaoSat(
        [FromQuery] Paging paging)
    {
        var leaveAllocations = await _mediator.Send(new GetBangKhaoSatConditionsRequest
    ...
    {
        var leaveAllocations = await _mediator.Send(new GetBangKhaoSatByDotKhaoSatRequest
            { PageIndex = paging.PageIndex, PageSize = paging.PageSize, Keyword = paging.Keyword });
        return Ok(leaveAllocations);
    }
